import { getLocationData, getTimeByLatLon } from "@/lib/location-info";
import LocationSwitcher from "./LocationSwitcher";
const LocationInfo = async ({ lat, lon }) => {
  const locationInfo = await getLocationData(lat, lon);
  // console.log(locationInfo);
  const timeInfo = await getTimeByLatLon(lat, lon);
  ...
  return (
    <div className="col-span-12 flex flex-col justify-end lg:col-span-8 2xl:col-span-9">
      <div>
        ...
        <div className="text-[#92B6F5] lg:text-sm">
          {/* <span>{new Date().toLocaleTimeString()}</span>{" "}
          <span>{new Date().toLocaleDateString()}</span> */}
          <span className="text-md">{timeInfo}</span>
        </div>
      </div>
    </div>
  );
